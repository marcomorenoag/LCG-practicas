...
int main() {
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		modelTemp = model = glm::translate(model, glm::vec3(1.5f, 0.0f, 0.0f));			//Matriz temporal "modelTemp" y"modelTemp2"
		model = glm::scale(model, glm::vec3(3.0f, 1.0f, 1.0f));
		color = glm::vec3(0.0f, 1.0f, 0.0f);											//Asignación de color con el nuevo shader.h
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		//Dedo 1
		model = glm::translate(modelTemp, glm::vec3(0.25f, 0.35f, 0.375f));				//Primera traslación para el punto pivote
		model = glm::rotate(model, glm::radians(dedo1), glm::vec3(0.0f, 0.0, 1.0f));
		modelTemp = model = glm::translate(model, glm::vec3(0.5f, 0.0f, 0.0f));			//Segunda traslación para el centro del nuevo cubo (geometria)
		...
		//Dedo 2
		model = glm::translate(modelTemp2, glm::vec3(0.25f, 0.35f, 0.0f));				//Se usa modelTemp2 porque es el pivote de la muñeca, así no dependerá del anterior dedo
		model = glm::rotate(model, glm::radians(dedo1), glm::vec3(0.0f, 0.0, 1.0f));
		...
		//Dedo 3
		model = glm::translate(modelTemp2, glm::vec3(0.25f, 0.35f, -0.375f));			//Se usa modelTemp2 porque es el pivote de la muñeca, así no dependerá del anterior dedo
		model = glm::rotate(model, glm::radians(dedo1), glm::vec3(0.0f, 0.0, 1.0f));
		modelTemp = model = glm::translate(model, glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(2.0f, 0.3f, 0.25f));
		color = glm::vec3(0.0f, 1.0f, 1.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		...
		//Dedo 4 (Pulgar)
		model = glm::translate(modelTemp2, glm::vec3(0.0f, 0.35f, -0.375f));			//Se usa modelTemp2 porque es el pivote de la muñeca, así no dependerá del anterior dedo
 ...
 void Inputs(GLFWwindow *window) {
	 ...
	 if (glfwGetKey(window, GLFW_KEY_H) == GLFW_PRESS)
		 muneca -= 0.18f;
	 /*A partir de aquí van los dedos*/
